...
product_blueprint = Blueprint(
    "product_blueprint", __name__, url_prefix="/products"
)
...
def find_all():
    ...
    products: list[
        ProductModel
    ] = product_repository.filtered_sorted_product_list(query_param)
    return (
        ProductResponseSchema(
            items=[
...
def new_product(scope):
    ...
    product = product_repository.create(
        ProductRequestSchema.parse_obj(request.json)
    )
...
def update_product(scope, id: int):
    ...
    product = product_repository.update(
        id, ProductRequestSchema.parse_obj(request.json)
    )
    if not product:
        raise ProductNotFoundException
