...
from app.orm.repository import (
    cart_repository,
...
def create_order(cart_uid):
    ...
    group(
        [
            send_admin_email.s(order.id, settings.admin_email),
            send_user_email.s(order.id, user_email),
        ]
    )()
...
def find_all(scope):
    ...
    return (
        jsonify(
            [
                OrderResponseSchemaWithItems.from_orm(order).dict()
                for order in orders
            ]
        ),
        http.HTTPStatus.OK,
...
def show_order(scope, id: int):
    ...
    return (
        jsonify(OrderResponseSchemaWithItems.from_orm(order).dict()),
        http.HTTPStatus.OK,
    )
