...
def login():
    user = user_repository.find_by_email(
        UserSchemaRequest.parse_obj(request.json).email
    )
    if not user:
        raise UserNotFoundException
    if not user.verify_password(request.json["password"]):
        raise Unauthorized
    role_id = 1 if user.admin else 2
    cart = user.cart
    if not cart:
        cart = cart_repository.create(CartSchema(user_id=user.id))
    payload = Payload(uid=str(cart.uid), role_id=role_id)
    token = encode_auth_token(payload)
    token_repository.create(
        TokenRequestSchema(
            access_token=payload.jti, user_id=user.id, exp=payload.exp
        )
    )
    return TokenResponse(access_token=token).json()
