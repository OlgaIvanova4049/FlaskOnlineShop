...
from app.orm.repository import category_repository, product_repository
...
from app.orm.schemas.response.product.category_response import (
    CategoryChildrenResponseSchema,
    CategoryResponseSchema,
)
from app.orm.schemas.response.product.product_response import (
...
def find_all_products(id: int):
    query_param: ProductQueryParam = ProductQueryParam.as_obj(
    ...
    return (
        ProductResponseSchema(
            ...
            paginator=query_param.paginator,
        ).dict(),
        http.HTTPStatus.OK,
    )
...
@swag_from("develop/category_list.yml")
def find_all():
    categories = category_repository.find_all()
    return (
        jsonify(
            [
                CategoryChildrenResponseSchema.from_orm(category).dict()
...
def new_category(scope):
    ...
    category = category_repository.create(
        CategoryRequestSchema.parse_obj(request.json)
    )
    return (
        CategoryResponseSchema.from_orm(category).json(),
...
def update_category(scope, id: int):
    ...
    category = category_repository.update(
        id, CategoryRequestSchema.parse_obj(request.json)
    )
    return (
        CategoryChildrenResponseSchema.from_orm(category).json(),
...
def delete_category(scope, id: int):
    ...
    return (
        jsonify({"message": "Category was successfully deleted"}),
